...
import { Location, NewCalendar } from "../../models/calendar";
...
const LessonForm = (props: LessonFromProps) => {
  ...
  const { mutate: addLesson } = useMutation(calendarsApi.create, {
    onSuccess: () => {
  ...
  const onSubmit = (data: NewCalendar) => {
      ...
      <Grid container spacing={2}>
        ...
        <Grid xs={12}>
          <Controller
            ...
            control={control}
            render={({ field }) => (
              <ImageUploader field={{ ...field }} defaultImage={undefined} />
            )}
          />
        </Grid>
      </Grid>
